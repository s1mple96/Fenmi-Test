...
class MySQLUtil:
    ...
    @staticmethod
    def get_table_fields(config: dict, database: str, table: str) -> List[Dict[str, Any]]:
        """
        获取指定表的所有字段信息，返回字段结构列表
        :param config: MySQL连接配置
        :param database: 数据库名
        :param table: 表名
        :return: 字段信息列表，每项为dict，含Field、Type、Extra等
        """
        config = config.copy()
        config['database'] = database
        db = MySQLUtil(**config)
        db.connect()
        fields = db.query(f'SHOW FULL COLUMNS FROM `{table}`')
        db.close()
        return fields

    @staticmethod
    def get_table_primary_keys(config: dict, database: str, table: str) -> List[str]:
        """
        获取指定表的主键字段名列表
        :param config: MySQL连接配置
        :param database: 数据库名
        ...
        db = MySQLUtil(**config)
        db.connect()
        keys = db.query(f"SHOW KEYS FROM `{table}` WHERE Key_name = 'PRIMARY'")
        db.close()
